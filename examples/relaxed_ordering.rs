...
fn main() {
    ...
    println!("Running {trials} trials with {elements} elements each...\n");
    ...
    for _trial in 0..trials {
        ...
        while let Some((key, _)) = spray.delete_min() {
            position_stats.entry(key).or_default().push(position);
            position += 1;
        }
    }
    ...
    for key in 1..=10 {
        ...
        if let Some(positions) = position_stats.get(&key) {
            ...
            println!("{key:3} | {avg:15.2} | {min:12} | {max:12}");
        }
    }
    ...
    for threads in [1, 2, 4, 8, 16] {
        ...
        println!("Threads: {threads:2} - First 5 deletions: {first_few:?}");
